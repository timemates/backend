...
import io.timemates.backend.integrations.postgresql.repositories.tables.TimerActivityTable
import io.timemates.backend.integrations.postgresql.repositories.tables.TimerParticipantsTable
import io.timemates.backend.integrations.postgresql.repositories.tables.TimersTable
    ...
    suspend fun removeMember(userId: Int, timerId: Int) =
        newSuspendedTransaction(db = database) {
            TimerParticipantsTable.deleteWhere {
                PARTICIPANT_ID eq userId and
                    (TIMER_ID eq timerId)
            }
        }

    suspend fun removeTimer(timerId: Int) = newSuspendedTransaction(db = database) {
        TimersTable.deleteWhere { TIMER_ID eq timerId }
    }
